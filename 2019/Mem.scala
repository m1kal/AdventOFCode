class Mem(val intcode: IntCode) extends java.io.Serializable with java.lang.Cloneable {
  var a: Array[Long] = Array[Long]()
  var breakpoints: Array[Int] = Array()
  def setBreakpoint(ad:Int)={breakpoints :+= ad}
  def apply(ad:Int) = {
    if ( intcode.pc < 1600 && breakpoints.contains(ad)) {println(s"R $ad: ${a(ad)}" +
            s" PC: ${intcode.pc} stack ${intcode.base} ${a.slice(intcode.base-4,intcode.base+4).toList} ${a(593)} ${a(753)}")}
    a(ad)}
  def update(ad:Int, v: Long) = {
    if (intcode.pc < 1600 && breakpoints.contains(ad)) {println(s"W $ad: $v" + 
            s" PC: ${intcode.pc} stack ${intcode.base} ${a.slice(intcode.base-4,intcode.base+4).toList} ${a(593)} ${a(753)}")}
    a(ad) = v
  }
  def slice(ad:Int, b:Int) = a.slice(ad,b)
}
object Mem {
  def apply[T](i:Array[Long], intcode: IntCode) = {
      val b = new Mem(intcode)
      b.a = i
      b
  }
}
